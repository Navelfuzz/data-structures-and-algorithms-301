...
const hasChildrenValues = (arr, character) => {
  return arr.some((char) => char.children && char.children.length > 0 && Object.values(char).includes(character));
};
...
const hasChildrenEntries = (arr, character) => {
  return arr.some((char) => {
    return Object.entries(char).some(([key, value]) => {
      return key === 'children' && value.includes(character);
    });
  });
};
...
const houseSize = (arr) => {
  ...
  arr.forEach((character) => {
    ...
    const members = 1 + (children ? children.length : 0) + (spouse ? 1 : 0);
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((character) => {
    const { house, children, spouse } = character;
    const member = 1 + (children ? children.length : 0) + (spouse && !deceasedSpouses.includes(spouse) ? 1 : 0);
...
describe('Testing challenge 5', () => {
  test('It should return an array of the names of the houses', () => {
    ...
    expect(getHouses(characters).length).toStrictEqual(6);
  });
});
