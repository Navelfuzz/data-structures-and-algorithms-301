...
let biggerThanLuke = (arr) => {
  ...
  if (luke && luke.mass !== undefined) {
    const lukeMass = parseInt(luke.mass);
    const biggerCharacters = arr.filter(character => character.mass && parseInt(character.mass) > lukeMass);
...
const detectTicTacToeWin = (board) => {
  ...
  const checkWinningCombination = (combination) => {
  ...
  for (const combination of winningCombinations) {
    if (checkWinningCombination(combination)) {
      return true;
    }
  }
